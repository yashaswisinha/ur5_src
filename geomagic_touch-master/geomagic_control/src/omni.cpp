...
public:
	...
	void init(OmniState *s) {
		...
		force_feedback_topic << "force_feedback";
 		//haptic_pub= n.advertise<geomagic_control::OmniFeedback>(haptic_topic.str(),100);
...
int main(int argc, char** argv) {
	...
	HDErrorInfo error;
	HHD hHD;
	hHD = hdInitDevice(HD_DEFAULT_DEVICE);//use ros param and set in launch file
	if (HD_DEVICE_ERROR(error = hdGetError())) {
